...
export default function BracketMatch({ t1, t2, label, winner }: BracketMatchProps) {
  ...
  return (
    <div style={{ background:THEME.bgCard,border:`1px solid ${THEME.borderCard}`,borderRadius:6,overflow:"hidden" }}>
      <div style={{ display:"flex",alignItems:"center",borderBottom:`1px solid ${THEME.borderMuted}` }}>
        <span style={{ fontSize:8,color:THEME.textFaint,padding:"0 5px",borderRight:`1px solid ${THEME.borderMuted}`,fontWeight:700,letterSpacing:0.5,whiteSpace:"nowrap" }}>{label}</span>
      ...
      <div style={{ display:"flex",alignItems:"center" }}>
        <span style={{ fontSize:8,color:THEME.bgCard,padding:"0 5px",borderRight:`1px solid ${THEME.borderMuted}`,fontWeight:700 }}>{label}</span>
