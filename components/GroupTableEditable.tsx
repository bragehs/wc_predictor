...
export default function GroupTableEditable({
  ...
  return (
    <div style={{ background:THEME.bgCard,borderRadius:8,overflow:"hidden",border:`1px solid ${THEME.borderCard}`,fontSize:12 }}>
      <div style={{ display:"grid",gridTemplateColumns:"18px 1fr 26px 26px 26px 38px 44px",padding:"4px 10px",background:THEME.bgInset,color:THEME.textMuted,fontSize:9,letterSpacing:1,textTransform:"uppercase" }}>
      ...
      {effective.map((row, i) => {
        ...
        return (
          <div key={row.team} style={{ display:"grid",gridTemplateColumns:"18px 1fr 26px 26px 26px 38px 44px",padding:"5px 10px",background:i%2===0?THEME.bgRow:THEME.bgRowAlt,borderTop:`1px solid ${THEME.borderFaint}`,alignItems:"center" }}>
